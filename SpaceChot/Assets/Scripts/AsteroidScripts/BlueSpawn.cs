...
{ //Atributos
    ...
    IEnumerator SpawnAsteroids(float time)
    {
        ...
        if (posiciones == 0)
        {
            Instantiate(BlueASTER, posicionAster[0].position, Quaternion.identity);
        }
        else
        {
            Instantiate(BlueASTER, posicionAster[1].position, Quaternion.identity);
        }
        PuedeSpawnear = false;
